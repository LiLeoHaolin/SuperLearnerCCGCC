...
for (m in seq){
  ...
  for (j in 1:fold){
    ...
    cox.fit = cph(formula.cc , data = dat.train, weights = weight, surv=T)
  ...
  for (j in 1:fold){
    ...
    dat.test = dat[dat$fold==j, ]
    dat.test = dat.test[is.na(dat.test$X10)==F,]
    exp.fit = flexsurvreg(formula.cc, data = dat.train, weights = weight, dist="exp")
    for (i in 1:nrow(dat.test)){
      subid = as.numeric(dat.test$subid[i])
  ...
  for (j in 1:fold){
    ...
    dat.test = dat[dat$fold==j, ]
    dat.test = dat.test[is.na(dat.test$X10)==F,]
    gam.fit = flexsurvreg(formula.cc, data = dat.train, weights = weight, dist="gamma")
    for (i in 1:nrow(dat.test)){
      subid = as.numeric(dat.test$subid[i])
  ...
  fn <- function(x){
    fn = sum((as.matrix(ind[,-1])-(x[1]*as.matrix(exp.survival.pred[,-1]) + x[2]*as.matrix(gam.survival.pred[,-1]) + x[3]*as.matrix(cox.survival.pred[,-1])+
                                     ...
                                     x[7]*as.matrix(cox2.survival.pred[,-1])+ x[8]*as.matrix(srf2.survival.pred[,-1]) ))^2 * (as.matrix(frac[,-1]))* (as.matrix(wt[,-1]))) / (n)
    fn
  } 
  ...
  exp.fit.pa = flexsurvreg(formula.cc, data = dat.cc, weights = weight, dist="exp")
  ...
  gam.survival.pa = data.frame(subid = rep(NA, nrow(dat.pa)), t1 = rep(NA, nrow(dat.pa)), t2 = rep(NA, nrow(dat.pa)), 
                               ...
                               t9 = rep(NA, nrow(dat.pa)), t10 = rep(NA, nrow(dat.pa)), t11 = rep(NA, nrow(dat.pa)),
                               t12 = rep(NA, nrow(dat.pa)), t13 = rep(NA, nrow(dat.pa)), t14 = rep(NA, nrow(dat.pa)))
  gam.fit.pa = flexsurvreg(formula.cc, data = dat.cc, weights = weight, dist="gamma")
  for (i in 1:nrow(dat.pa)){
    subid = as.numeric(dat.pa$subid[i])
  ...
  cox.survival.pa = data.frame(subid = rep(NA, nrow(dat.pa)), t1 = rep(NA, nrow(dat.pa)), t2 = rep(NA, nrow(dat.pa)), 
                               ...
                               t9 = rep(NA, nrow(dat.pa)), t10 = rep(NA, nrow(dat.pa)), t11 = rep(NA, nrow(dat.pa)),
                               t12 = rep(NA, nrow(dat.pa)), t13 = rep(NA, nrow(dat.pa)), t14 = rep(NA, nrow(dat.pa)))
  cox.fit.pa = cph(formula.cc , data = dat.cc, weights = weight, surv=T)
  for (i in 1:nrow(dat.pa)){
    subid = as.numeric(dat.pa$subid[i])
...
write.csv(ipcw.wt.cum, file=paste0(path.results, 'cc_w_w_ipcw_wt_', model, '_', spec, '_', cen.mod, '_', batch, '.csv'), row.names = F)
write.csv(ipcwsl.pa.cum, file=paste0(path.results, 'cc_w_w_ipcwsl_pa_', model, '_', spec, '_', cen.mod, '_', batch, '.csv'), row.names = F)
