...
for (m in seq){
  ...
  dat = read.csv(paste0(path.r, 'dat', m, '.csv'))
  n = nrow(dat)
  dat$subid = c(1:n)
  dat$fold = 0
  for (i in 1:n){
...
write.csv(ipcw.wt.cum, file=paste0(path.results, 'r_ipcw_wt_', model, '_', spec, '_', cen.mod, '_', batch, '.csv'), row.names = F)
write.csv(ipcwsl.pa.cum, file=paste0(path.results, 'r_ipcwsl_pa_', model, '_', spec, '_', cen.mod, '_', batch, '.csv'), row.names = F)
