...
for (m in seq){
  ...
  for (j in 1:fold){
    ...
    cb.fit = CoxBoost(time=dat.train$time, status=dat.train$status, x=as.matrix(dat.train[,1:p])) 
  ...
  for (j in 1:fold){
    ...
    dat.test = dat[dat$fold==j, ]
    srf.fit = rfsrc(Surv(time, status) ~ ., data=dat.train)
    for (i in 1:nrow(dat.test)){
  ...
  cb.survival.pa = data.frame(subid = rep(0, nrow(dat.pa)), t1 = rep(0, nrow(dat.pa)), t2 = rep(0, nrow(dat.pa)), 
                         ...
                         t9 = rep(0, nrow(dat.pa)), t10 = rep(0, nrow(dat.pa)), t11 = rep(0, nrow(dat.pa)),
                         t12 = rep(0, nrow(dat.pa)), t13 = rep(0, nrow(dat.pa)), t14 = rep(0, nrow(dat.pa)))
  cb.fit.pa = CoxBoost(time=dat$time, status=dat$status, x=as.matrix(dat[,1:p]))
  ...
  srf.survival.pa = data.frame(subid = rep(0, nrow(dat.pa)), t1 = rep(0, nrow(dat.pa)), t2 = rep(0, nrow(dat.pa)), 
                          ...
                          t9 = rep(0, nrow(dat.pa)), t10 = rep(0, nrow(dat.pa)), t11 = rep(0, nrow(dat.pa)),
                          t12 = rep(0, nrow(dat.pa)), t13 = rep(0, nrow(dat.pa)), t14 = rep(0, nrow(dat.pa)))
  srf.fit <-  rfsrc(Surv(time, status) ~ ., data = dat.learn)
    for (i in 1:nrow(dat.pa)){
...
write.csv(cb.pa.cum, file=paste0(path.results, 'c_large_cb_pa_', model, '_', spec, '_', cen.mod, '_', batch, '.csv'), row.names = F)
