...
for (m in seq){
  ...
  true.lin.pred <- as.matrix(dat.pa[,1:p]) %*% beta + 0.3*as.matrix(dat.pa[,139])*as.matrix(dat.pa[,141]) - 0.3*as.matrix(dat.pa[,141])^2 + 0.2*as.matrix(dat.pa[,139])*as.matrix(dat.pa[,144])*as.matrix(dat.pa[,147]) - 0.4*as.matrix(dat.pa[,140])*as.matrix(dat.pa[,142])*as.matrix(dat.pa[,143])
  #+ 0.3*X[,1]*X[,3] + 0.2*X[,3]^2 + 0.2*X[,1]*X[,6]*X[,9]
  ...
  cb.fit.pa = CoxBoost(time=dat.cc$time, status=dat.cc$status, x=as.matrix(dat.cc[,1:p])) 
...
write.csv(cb.pa.cum, file=paste0(path.results, 'r_cb_pa_', model, '_', spec, '_', cen.mod, '_', batch, '.csv'), row.names = F)
write.csv(srf.pa.cum, file=paste0(path.results, 'r_srf_pa_', model, '_', spec, '_', cen.mod, '_', batch, '.csv'), row.names = F)
write.csv(cb2.pa.cum, file=paste0(path.results, 'r_cb2_pa_', model, '_', spec, '_', cen.mod, '_', batch, '.csv'), row.names = F)
write.csv(srf2.pa.cum, file=paste0(path.results, 'r_srf2_pa_', model, '_', spec, '_', cen.mod, '_', batch, '.csv'), row.names = F)
