...
path.test <- "/nas/longleaf/home/haolin/dissertation1/round8/data/test/"
path.c <- "/nas/longleaf/home/haolin/dissertation1/round8/data/c/"
#path.cc <- "/nas/longleaf/home/haolin/dissertation1/round8/data/cc/"
#path.r <- "/nas/longleaf/home/haolin/dissertation1/round8/data/r/"
#path.sc <- "/nas/longleaf/home/haolin/dissertation1/round8/data/sc/"
path.results <- "/nas/longleaf/home/haolin/dissertation1/round8/results/"
...
for (m in seq){
  ...
  cb.survival.pred = data.frame(subid = rep(0, n), t1 = rep(0, n), t2 = rep(0, n), t3 = rep(0, n), t4 = rep(0, n),
  ...
  for (j in 1:fold){
    ...
    for (i in 1:nrow(dat.test)){
      ...
      cb.survival.pred$t1[subid] = 1-predict(cb.fit, newdata = as.matrix(dat.test[i, 1:p.full]),  time = 1*tau/10, type = "CIF")
      cb.survival.pred$t2[subid] = 1-predict(cb.fit, newdata = as.matrix(dat.test[i, 1:p.full]),  time = 2*tau/10, type = "CIF")
      cb.survival.pred$t3[subid] = 1-predict(cb.fit, newdata = as.matrix(dat.test[i, 1:p.full]),  time = 3*tau/10, type = "CIF")
      cb.survival.pred$t4[subid] = 1-predict(cb.fit, newdata = as.matrix(dat.test[i, 1:p.full]),  time = 4*tau/10, type = "CIF")
      cb.survival.pred$t5[subid] = 1-predict(cb.fit, newdata = as.matrix(dat.test[i, 1:p.full]),  time = 5*tau/10, type = "CIF")
      cb.survival.pred$t6[subid] = 1-predict(cb.fit, newdata = as.matrix(dat.test[i, 1:p.full]),  time = 6*tau/10, type = "CIF")
      cb.survival.pred$t7[subid] = 1-predict(cb.fit, newdata = as.matrix(dat.test[i, 1:p.full]),  time = 7*tau/10, type = "CIF")
      cb.survival.pred$t8[subid] = 1-predict(cb.fit, newdata = as.matrix(dat.test[i, 1:p.full]),  time = 8*tau/10, type = "CIF")
      cb.survival.pred$t9[subid] = 1-predict(cb.fit, newdata = as.matrix(dat.test[i, 1:p.full]),  time = 9*tau/10, type = "CIF")
    }
  }

  
  # 2.2 Survival random forest #
  
  srf.survival.pred = data.frame(subid = rep(0, n), t1 = rep(0, n), t2 = rep(0, n), t3 = rep(0, n), t4 = rep(0, n),
                            t5 = rep(0, n), t6 = rep(0, n), t7 = rep(0, n), t8 = rep(0, n), t9 = rep(0, n))
  for (j in 1:fold){
    dat.train = dat[dat$fold!=j, ]
    #weight = dat.train$weight
    dat.train <- subset(dat.train, select = -c(subid, fold))
    dat.test = dat[dat$fold==j, ]
  ...
  fn <- function(x){
    fn = sum((as.matrix(ind[,-1])-(x[1]*as.matrix(cb.survival.pred[,-1]) + x[2]*as.matrix(srf.survival.pred[,-1])))^2 * (as.matrix(frac[,-1]))) / (n)
    fn
  } 
  ...
  true.lin.pred <- as.matrix(dat.pa[,1:p]) %*% beta + 0.3*as.matrix(dat.pa[,139])*as.matrix(dat.pa[,141]) - 0.3*as.matrix(dat.pa[,141])^2 + 0.2*as.matrix(dat.pa[,139])*as.matrix(dat.pa[,144])*as.matrix(dat.pa[,147]) - 0.4*as.matrix(dat.pa[,140])*as.matrix(dat.pa[,142])*as.matrix(dat.pa[,143])#+ 0.3*X[,1]*X[,3] + 0.2*X[,3]^2 + 0.2*X[,1]*X[,6]*X[,9]
  ...
  cb.survival.pa = data.frame(subid = rep(0, nrow(dat.pa)), t1 = rep(0, nrow(dat.pa)), t2 = rep(0, nrow(dat.pa)), 
                         t3 = rep(0, nrow(dat.pa)), t4 = rep(0, nrow(dat.pa)), t5 = rep(0, nrow(dat.pa)), 
                         t6 = rep(0, nrow(dat.pa)), t7 = rep(0, nrow(dat.pa)), t8 = rep(0, nrow(dat.pa)), 
                         t9 = rep(0, nrow(dat.pa)), t10 = rep(0, nrow(dat.pa)), t11 = rep(0, nrow(dat.pa)),
                         t12 = rep(0, nrow(dat.pa)), t13 = rep(0, nrow(dat.pa)), t14 = rep(0, nrow(dat.pa)))
  ...
  for (i in 1:nrow(dat.pa)){
    ...
    cb.survival.pa$t1[subid] = 1-predict(cb.fit.pa, newdata = as.matrix(dat.pa[i, 1:p.full]),  time = 1*tau/15, type = "CIF")
    cb.survival.pa$t2[subid] = 1-predict(cb.fit.pa, newdata = as.matrix(dat.pa[i, 1:p.full]),  time = 2*tau/15, type = "CIF")
    cb.survival.pa$t3[subid] = 1-predict(cb.fit.pa, newdata = as.matrix(dat.pa[i, 1:p.full]),  time = 3*tau/15, type = "CIF")
    cb.survival.pa$t4[subid] = 1-predict(cb.fit.pa, newdata = as.matrix(dat.pa[i, 1:p.full]),  time = 4*tau/15, type = "CIF")
    cb.survival.pa$t5[subid] = 1-predict(cb.fit.pa, newdata = as.matrix(dat.pa[i, 1:p.full]),  time = 5*tau/15, type = "CIF")
    cb.survival.pa$t6[subid] = 1-predict(cb.fit.pa, newdata = as.matrix(dat.pa[i, 1:p.full]),  time = 6*tau/15, type = "CIF")
    cb.survival.pa$t7[subid] = 1-predict(cb.fit.pa, newdata = as.matrix(dat.pa[i, 1:p.full]),  time = 7*tau/15, type = "CIF")
    cb.survival.pa$t8[subid] = 1-predict(cb.fit.pa, newdata = as.matrix(dat.pa[i, 1:p.full]),  time = 8*tau/15, type = "CIF")
    cb.survival.pa$t9[subid] = 1-predict(cb.fit.pa, newdata = as.matrix(dat.pa[i, 1:p.full]),  time = 9*tau/15, type = "CIF")
    cb.survival.pa$t10[subid] = 1-predict(cb.fit.pa, newdata = as.matrix(dat.pa[i, 1:p.full]),  time = 10*tau/15, type = "CIF")
    cb.survival.pa$t11[subid] = 1-predict(cb.fit.pa, newdata = as.matrix(dat.pa[i, 1:p.full]),  time = 11*tau/15, type = "CIF")
    cb.survival.pa$t12[subid] = 1-predict(cb.fit.pa, newdata = as.matrix(dat.pa[i, 1:p.full]),  time = 12*tau/15, type = "CIF")
    cb.survival.pa$t13[subid] = 1-predict(cb.fit.pa, newdata = as.matrix(dat.pa[i, 1:p.full]),  time = 13*tau/15, type = "CIF")
    cb.survival.pa$t14[subid] = 1-predict(cb.fit.pa, newdata = as.matrix(dat.pa[i, 1:p.full]),  time = 14*tau/15, type = "CIF")
  }
  ...
  #weight = dat$weight
  dat.learn <- subset(dat, select = -c(subid, fold))
  ...
  srf.survival.pa = data.frame(subid = rep(0, nrow(dat.pa)), t1 = rep(0, nrow(dat.pa)), t2 = rep(0, nrow(dat.pa)), 
                          ...
                          t9 = rep(0, nrow(dat.pa)), t10 = rep(0, nrow(dat.pa)), t11 = rep(0, nrow(dat.pa)),
                          t12 = rep(0, nrow(dat.pa)), t13 = rep(0, nrow(dat.pa)), t14 = rep(0, nrow(dat.pa)))
  srf.fit <-  rfsrc(formula.full, data = dat.learn)
    for (i in 1:nrow(dat.pa)){
      ...
      srf.survival.pa$t1[subid] = max(srf.pred[srf.pred$time1>0, 1])
      srf.survival.pa$t2[subid] = max(srf.pred[srf.pred$time2>0, 1])
      srf.survival.pa$t3[subid] = max(srf.pred[srf.pred$time3>0, 1])
      srf.survival.pa$t4[subid] = max(srf.pred[srf.pred$time4>0, 1])
      srf.survival.pa$t5[subid] = max(srf.pred[srf.pred$time5>0, 1])
      srf.survival.pa$t6[subid] = max(srf.pred[srf.pred$time6>0, 1])
      srf.survival.pa$t7[subid] = max(srf.pred[srf.pred$time7>0, 1])
      srf.survival.pa$t8[subid] = max(srf.pred[srf.pred$time8>0, 1])
      srf.survival.pa$t9[subid] = max(srf.pred[srf.pred$time9>0, 1])
      srf.survival.pa$t10[subid] = max(srf.pred[srf.pred$time10>0, 1])
      srf.survival.pa$t11[subid] = max(srf.pred[srf.pred$time11>0, 1])
      srf.survival.pa$t12[subid] = max(srf.pred[srf.pred$time12>0, 1])
      srf.survival.pa$t13[subid] = max(srf.pred[srf.pred$time13>0, 1])
      srf.survival.pa$t14[subid] = max(srf.pred[srf.pred$time14>0, 1])
  }
...
write.csv(ipcw.wt.cum, file=paste0(path.results, 'c_small_ipcw_wt_', model, '_', spec, '_', cen.mod, '_', batch, '.csv'), row.names = F)
write.csv(ipcwsl.pa.cum, file=paste0(path.results, 'c_small_ipcwsl_pa_', model, '_', spec, '_', cen.mod, '_', batch, '.csv'), row.names = F)

write.csv(cb.pa.cum, file=paste0(path.results, 'c_small_cb_pa_', model, '_', spec, '_', cen.mod, '_', batch, '.csv'), row.names = F)
write.csv(srf.pa.cum, file=paste0(path.results, 'c_small_srf_pa_', model, '_', spec, '_', cen.mod, '_', batch, '.csv'), row.names = F)
