...
#path.cc <- "C:/Users/Haolin Li/Desktop/Dissertation/04_project1/03_simulations/02_data_generation/RC/test/cc/"
#path.sc <- "C:/Users/Haolin Li/Desktop/Dissertation/04_project1/03_simulations/02_data_generation/RC/test/sc/"
#path.results <- "C:/Users/Haolin Li/Desktop/Dissertation/04_project1/07_simulation_new/data/test/"
...
for (m in seq){
  ...
  for (j in 1:fold){
    ...
    cox.fit = cph(Surv(time, status) ~ X1+X2, data = dat.train, surv=T)
  ...
  for (j in 1:fold){
    dat.train = dat[dat$fold!=j, ]
    dat.test = dat[dat$fold==j, ]
    exp.fit = flexsurvreg(Surv(time, status) ~ X1+X2, data = dat.train, dist="exp")
    for (i in 1:nrow(dat.test)){
      subid = as.numeric(dat.test$subid[i])
  ...
  for (j in 1:fold){
    dat.train = dat[dat$fold!=j, ]
    dat.test = dat[dat$fold==j, ]
    gam.fit = flexsurvreg(Surv(time, status) ~ X1+X2, data = dat.train, dist="gamma")
    for (i in 1:nrow(dat.test)){
      subid = as.numeric(dat.test$subid[i])
  ...
  for (j in 1:fold){
    dat.train = dat[dat$fold!=j, ]
    dat.test = dat[dat$fold==j, ]
    srf.fit = rfsrc(Surv(time, status) ~ X1+X2, data = dat.train)
    for (i in 1:nrow(dat.test)){
  ...
  beta = matrix(c(0.1, 0.1, 0.3, -0.3, -0.2, -0.3, 0.01, -0.01, 0.25, -0.25), nrow = 10)
  true.lin.pred <- as.matrix(dat.pa[,1:10]) %*% beta #+ 0.3*X[,1]*X[,3] + 0.2*X[,3]^2 + 0.2*X[,1]*X[,6]*X[,9]
  ...
  exp.survival.pa = data.frame(subid = rep(0, nrow(dat.pa)), t1 = rep(0, nrow(dat.pa)), t2 = rep(0, nrow(dat.pa)), 
                         ...
                         t9 = rep(0, nrow(dat.pa)), t10 = rep(0, nrow(dat.pa)), t11 = rep(0, nrow(dat.pa)),
                         t12 = rep(0, nrow(dat.pa)), t13 = rep(0, nrow(dat.pa)), t14 = rep(0, nrow(dat.pa)))
  exp.fit.pa = flexsurvreg(Surv(time, status) ~ X1+X2, data = dat, dist="exp")
  ...
  gam.survival.pa = data.frame(subid = rep(0, nrow(dat.pa)), t1 = rep(0, nrow(dat.pa)), t2 = rep(0, nrow(dat.pa)), 
                               ...
                               t9 = rep(0, nrow(dat.pa)), t10 = rep(0, nrow(dat.pa)), t11 = rep(0, nrow(dat.pa)),
                               t12 = rep(0, nrow(dat.pa)), t13 = rep(0, nrow(dat.pa)), t14 = rep(0, nrow(dat.pa)))
  gam.fit.pa = flexsurvreg(Surv(time, status) ~ X1+X2, data = dat, dist="gamma")
  for (i in 1:nrow(dat.pa)){
    subid = as.numeric(dat.pa$subid[i])
  ...
  cox.survival.pa = data.frame(subid = rep(0, nrow(dat.pa)), t1 = rep(0, nrow(dat.pa)), t2 = rep(0, nrow(dat.pa)), 
                               ...
                               t9 = rep(0, nrow(dat.pa)), t10 = rep(0, nrow(dat.pa)), t11 = rep(0, nrow(dat.pa)),
                               t12 = rep(0, nrow(dat.pa)), t13 = rep(0, nrow(dat.pa)), t14 = rep(0, nrow(dat.pa)))
  cox.fit.pa = cph(Surv(time, status) ~ X1+X2, data = dat, surv=T)
  for (i in 1:nrow(dat.pa)){
    subid = as.numeric(dat.pa$subid[i])
  ...
  srf.survival.pa = data.frame(subid = rep(0, nrow(dat.pa)), t1 = rep(0, nrow(dat.pa)), t2 = rep(0, nrow(dat.pa)), 
                          ...
                          t9 = rep(0, nrow(dat.pa)), t10 = rep(0, nrow(dat.pa)), t11 = rep(0, nrow(dat.pa)),
                          t12 = rep(0, nrow(dat.pa)), t13 = rep(0, nrow(dat.pa)), t14 = rep(0, nrow(dat.pa)))
  srf.fit <-  rfsrc(Surv(time, status) ~ X1+X2, data = dat)
    for (i in 1:nrow(dat.pa)){
...
write.csv(exp.pa.cum, file=paste0(path.results, 'c_small_exp_pa_', model, '_', spec, '_', cen.mod, '_', batch, '.csv'), row.names = F)
write.csv(gam.pa.cum, file=paste0(path.results, 'c_small_gam_pa_', model, '_', spec, '_', cen.mod, '_', batch, '.csv'), row.names = F)
write.csv(cox.pa.cum, file=paste0(path.results, 'c_small_cox_pa_', model, '_', spec, '_', cen.mod, '_', batch, '.csv'), row.names = F)
