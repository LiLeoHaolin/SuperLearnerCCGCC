...
for (m in seq){
  ...
  true.lin.pred <- as.matrix(dat.pa[,1:10]) %*% beta + 0.3*as.matrix(dat.pa[,1])*as.matrix(dat.pa[,3]) - 0.3*as.matrix(dat.pa[,3])^2 + 0.2*as.matrix(dat.pa[,1])*as.matrix(dat.pa[,6])*as.matrix(dat.pa[,9]) - 0.4*as.matrix(dat.pa[,2])*as.matrix(dat.pa[,4])*as.matrix(dat.pa[,5])+1.6
